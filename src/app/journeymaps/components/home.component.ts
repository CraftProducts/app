...
import { JourneymapsState } from '../+state/journeymaps.state';
import { ActivatedRoute, Router } from '@angular/router';
import { SelectSectionAction } from '../+state/journeymaps.actions';
...
import { extractSections } from '../journeymaps-utils';

@Component({
    selector: 'app-journeymaps-home',
    templateUrl: './home.component.html'
})
export class JourneymapsHomeComponent extends BaseTemplateViewer implements ComponentCanDeactivate, OnInit, OnDestroy {
    ...
    constructor(public store$: Store<JourneymapsState>, public router: Router, public activatedRoute: ActivatedRoute) {
    ...
    ngOnInit() {
        ...
        this.model$ = this.store$.select(p => p.journeymaps.modelInstance)
            .subscribe(model => this.model = model);

        this.selectedSection$ = this.store$.select(p => p.journeymaps.selectedSection)
